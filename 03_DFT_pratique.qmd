...
Dans le premier exemple, nous allons considérer un filtre RLC serie. Sur ce filtre, nous voulons calculer la fonction de transfert entre la tension source $V_e$ et la tension au bord de la capacitance $V_c$. Nous obtenons la fonction de transfert suivante :
...
Nous en déduisons la réponse impulsionnelle de cette fonction de transfert :
...
```{python}
H_ref_1 = H(2 * np.pi * f)
H_ref_2 = H(2 * np.pi * (f - 1/Te))

fig, axs = plt.subplots(2)
axs[0].loglog(f, np.abs(H_ref_1), label="$H(f)$")
axs[0].loglog(f, np.abs(H_ref_2), label="$H(f-f_e)$")
axs[0].loglog(f, np.abs(H_ref_1 + H_ref_2), label="$H(f) + H(f-f_e)$")
axs[0].loglog(f, np.abs(H_dtft), label="DTFT")
axs[0].grid()
axs[0].legend()

axs[1].semilogx(f, np.angle(H_ref_1), label="$H(f)$")
axs[1].semilogx(f, np.angle(H_ref_2), label="$H(f-f_e)$")
axs[1].semilogx(f, np.angle(H_ref_1 + H_ref_2), label="$H(f) + H(f-f_e)$")
axs[1].semilogx(f, np.angle(H_dtft), label="DTFT")
axs[1].grid()
```
...
```{python}
from smux.dft import dftfreq

f = dftfreq(N, Te)
f_max = f[-1]

fig, amp = plt.subplots(1)
...
for fe in (2 * f_max, 6 * f_max, 20 * f_max, 60 * f_max):
    ...
    lines = amp.semilogy(
        f,
        np.abs(H_dtft),
        label="$DTFT(h)(f_e / f_{max} = %g)$" % (fe / f_max),
    )
amp.semilogy(
    f,
    np.abs(H_ref),
...
def dirac_comb_ft(ft, freq, time, dtype=complex):
    ...
    h = np.zeros_like(freq, dtype=dtype)
    fe = 1.0 / time[1]
    for i in range(min(len(time), 50)):
        h += ft(freq - i * fe)
    return h
...
fig, (err_abs, err_rel) = plt.subplots(2)
